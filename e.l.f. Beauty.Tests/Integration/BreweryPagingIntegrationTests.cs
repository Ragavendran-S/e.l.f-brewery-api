        ...
        public async Task GetBreweriesAsync_ReturnsPage2_WhenDbHasMultiplePages()
        {
            ...
            var clientFactory = _factory.WithWebHostBuilder(builder =>
            {
                builder.ConfigureServices(services =>
                {
                    ...
                    // Replace DbContext registration with an in-memory DB for this test
            ...
            // Seed using the test server's service provider so the request pipeline
            // observes the same in-memory DB instance. Do this before creating the
            // HTTP client to ensure the data is available during the request.
